...
if uploaded_file is not None:
    ...
    # Display phone numbers if names are missing
    user_list = df['user'].unique().tolist()
    user_list = [user if not user.isnumeric() else f"User {user}" for user in user_list]
    ...
    # Match back to phone number if "User +91XXXX" is selected
    selected_user = selected_user.replace("User ", "")

    if st.sidebar.button("Show Analysis"):
        filtered_df = helper.filter_data(selected_user, df)
